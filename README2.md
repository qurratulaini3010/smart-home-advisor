...
- **Two kinds of input handling in `index.php`:**
  1. **Persistent form POSTs** — a `<form method="post">` with a hidden `<input name="action" value="...">`. These are read starting at the block `if ($dbError === null && $_SERVER['REQUEST_METHOD'] === 'POST')` (around line 262 of `index.php`). Each `if ($action === '...')` block is one handler. These always end in either `redirect()` (send the browser to a new `?page=`) or a flash message.
...
|---|---|---|---|
| `landing` (default if logged out) | No | Marketing/home page | `index.php` ~698 |
| `login` | No | Login form | ~749 |
| `register` | No | Registration form | ~749 |
| `forgot` | No | Static "ask admin" notice | ~749 |
| `dashboard` (default if logged in) | Yes | User stats + favorites | ~812 |
| `assessment` | Yes | 4-step assessment wizard | ~858 |
| `results` | Yes | Ranked property matches for one assessment | ~1038 |
| `property_directory` | Yes | Searchable property grid (delegates to `public/partials/property_directory_tabs.php`) | ~1113 |
| `property` | Yes | Full property detail page | ~1121 |
| `mortgage` | Yes | Standalone mortgage calculator | ~1317 |
| `profile` | Yes | Edit name/phone/DOB/occupation | ~1335 |
| `financial_profile` | Yes | Income + commitments manager | ~1382 |
| `admin_dashboard` | Admin | Admin stat tiles | ~1546 |
| `admin_users` | Admin | User list + role changer | ~1561 |
| `admin_properties` | Admin | Add/edit/delete properties | ~1571 |
| `admin_reports` | Admin | Assessment history table + CSV export | ~1650 |
| `export_reports` | Admin | Not a page — streams a CSV file | ~588 |
| `logout` | — | No UI — clears session, redirects to `landing` | ~581 |
...
> **Note:** this field only renders on the register form now — it used to also render (incorrectly) on the login form. See the "Recent fix" note at the bottom of this document.
...
> **Why income/commitments live here and not on the assessment form:** `assessment_store` (below) pulls `gross_monthly_income` and the sum of `user_commitments` straight from the database — it does **not** read them from any assessment form field. If `gross_monthly_income` is 0, the assessment page blocks you with a banner telling you to fill this out first.

---

## 6. Assessment Wizard — `page=assessment`

This is the most involved flow in the app: one visible 4-step client-side wizard, one hidden "real" submission form, one live-preview AJAX call, and a handful of inputs that are collected but currently **not used by anything** (flagged below so you don't go looking for where they're consumed and assume you missed something).
...
- A **hidden form** (`#assessmentFinalForm`, `action=assessment_store`) is what actually gets POSTed. `app.js` copies values from the visible wizard into this hidden form's fields right before submitting.
...
| `tenure_preference` | select | ❌ **no** | ❌ **no** |
| `bedrooms` | number | ❌ **no** | ❌ **no** |
| `low_flood_risk` | checkbox | ❌ **no** | ❌ **no** |
| `near_school` | checkbox | ❌ **no** | ❌ **no** |

**Important:** `tenure_preference`, `bedrooms`, `low_flood_risk`, and `near_school` are collected by `collectAssessmentData()` (they carry `data-assessment-field`) and are sent in the live-preview request, but neither `assessment_preview` (in `index.php`) nor `copyAssessmentToFinalForm()` (in `app.js`) reads those four keys. They currently have **no effect on scoring, ranking, or the saved assessment** — filling them in changes nothing. This is a real gap in the current build, not a documentation simplification: these four inputs exist as UI elements only. (The same four preferences *are* wired up and functional in a different part of the codebase — see §8.2's "recommend" advisor, which is currently unreachable from any page.)
...
| `sustainability_priority_slider` | 0–100, default 50 | **❌ collected but never read — no effect on anything** |
| `family_priority_slider` | 0–100, default 70 | if ≥ 60 → `comfort_priority = "Family growth"` |
| `quiet_priority_slider` | 0–100, default 50 | if `family` < 60 and this ≥ 60 → `comfort_priority = "Acoustic comfort"`; otherwise comfort_priority falls through to `"Energy efficiency"` |
...
**Goes to:** `POST index.php?action=assessment_preview` (handled inside `index.php`, *not* a separate file — look for the `assessment_preview` block near the top, before the persistent-form section).
**Sends:** `budget`, `monthly_income` *(always empty from this form — see note)*, `monthly_commitment` *(always empty — see note)*, `preferred_location`, `property_type`, `comfort_priority`, and the four `smart_*` flags derived from the sliders.

> **Note on income during preview:** the visible wizard has no income/commitment fields (those live on the Financial Profile page), so `monthly_income`/`monthly_commitment` are always `0` in this AJAX call. The server compensates by ignoring the posted income entirely for the *occupation* lookup (it re-fetches `occupation` fresh from `users` for the logged-in user) but the affordability-tilt math inside the preview still runs on a net income of `0`. In practice this means the **preview's affordability-driven ranking can differ slightly from the final saved results**, which do use your real `gross_monthly_income` minus commitments. The match percentages you see in Step 4 are directionally correct (budget filtering, location filtering, smart-feature scoring, and occupation adjustments all work identically) but the income-based affordability bonus/penalty tiers won't apply until you save.
...
|---|---|
| `budget` | Step 1 |
| `preferred_location` | Step 2 |
| `property_type` | Step 2 |
...
**Server logic (`index.php`, `if ($action === 'assessment_store')`):**
...
4. Requires `budget > 0`.
5. `INSERT INTO assessments (...)` with all of the above.
6. Calls `save_recommendations($assessmentId, $assessment)`, which:
   ...
   - Scores every candidate property with `RecommendationEngine::score()` (see §9 for the formula).
...
`property-advisor.js` also contains a second flow: a 3-step wizard (`#advisorWizardForm`, `#advisorNext`, `#advisorBack`, `#advisorResults`, `#favoriteSuccess`) that would call `action=recommend` and a "Love It ❤️" favorite button on each advisor result card. **None of these element IDs currently exist anywhere in `property_directory_tabs.php` or any other template**, so this code path never runs — the relevant `document.getElementById(...)` calls simply return `null` and the optional-chained listeners never attach. If you want to enable it, the wizard markup needs to be added back to the partial; the API side (`action=recommend` in `property-directory.php`, using `PropertyDirectoryRepository::recommend()`) is fully implemented and does use `tenure_preference`, `bedrooms`, `low_flood_risk`, `near_school`, and the five `*_priority` weights — it's a **second, independent scoring formula** from the one used by the assessment wizard (§9), tuned by the admin-adjustable-in-spirit-but-actually-hard-coded default weight of 50 for each dimension unless the caller supplies otherwise.
...
**Rule badges** (`#propertyRuleBadges`): on page load, an inline `<script>` fires `GET api/rules.php?property_id=<id>&budget=<your last assessment's budget>` and renders whatever badges come back (see §10.3).
...
- **Security** = property's base security score, +8 if you flagged `smart_security`.
...
- **Environment** = property's sustainability score, +7 if `smart_energy` flagged, +4 for high-income/low-commitment profiles, then a retiree-specific hospital-proximity bonus or penalty.
- **Family** = property's base family score, +8 if household size ≥ 4 and the property has ≥3 bedrooms *(note: household size is no longer collected anywhere in the current UI, so this branch is effectively always using whatever default/legacy value is in the assessment row)*, +6 for retirees on high-safety properties, +5/−10 for location/type match or mismatch.
...
A **second, independent** rules engine (`public/api/rules.php`, ~108 rules across 8 domains) runs separately to generate the plain-language badges you see on property detail pages and directory modals — it does not feed back into `match_percentage` at all; it's purely explanatory.
